...
class DashboardActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        db = DBHelper(this)
        user = db.getUserById(SessionManager(this).getUserId())
        ...
        adapter = OfferAdapter(emptyList(), canBook) { offer ->
            startActivity(Intent(this, BookingConfirmationActivity::class.java)
                .putExtra(BookingConfirmationActivity.EXTRA_OFFER_ID, offer.id))
        }
        ...
        val isAdmin = user!!.role == Roles.ADMIN
        ...
        findViewById<MaterialButton>(R.id.btnManageUsers).apply {
            visibility = if (isAdmin) View.VISIBLE else View.GONE
            setOnClickListener { startActivity(Intent(this@DashboardActivity, AdminUsersActivity::class.java)) }
        }
